...
describe('GET /staff/unstaffed', () => {
	it('should not allow unauthorised access', async () => {
		const res = await request(app).get('/api/v1/staff/unstaffed');
		expect(res.status).toBe(401);
	});

	it('should not allow users to view the route', async () => {
		const res = await request(app)
			.get('/api/v1/staff/unstaffed')
			.set({ Authorization: `Basic ${btoa('TestUser:userPass')}` });
		expect(res.status).toBe(403);
	});

	it('should allow staff to view unstaffed locations', async () => {
		const res = await request(app)
			.get('/api/v1/staff/unstaffed')
			.set({ Authorization: `Basic ${btoa('TestStaff:staffPass')}` });
		expect(res.status).toBe(200);
